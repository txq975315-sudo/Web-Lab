...
【项目约束清单（可选）】
以下条目来自用户在项目树「01 项目宪法」模块中自行维护的硬性约束；若显示「尚未录入」则表示未配置，不得臆造条目或称为「宪法第几条」。
{{project.constraints}}
...
### 2. 产品逻辑审计
- 结合上文「目标用户」「差异化」「明确反对」与「项目约束清单」（若有），对用户自述或已给出的平台、交互、数据与规模等**真实约束**做压力测试
...
现在开始与用户对话。`.trim() +
  ...
  `\n四个 ### 模块内：已有示例列表须保留列表形态；若补充论述，仍须用「- 」分条写出，禁止在同一模块内输出无列表的长段落堆砌。`

const GROWTH_COACH_PROMPT_BODY = `你是一位专注于商业洞察力和产品思维的成长教练。你的任务不是替代用户思考，而是通过刻意练习帮助他成长。
...
export function buildLiveLabSystemPrompt(project = {}, mode = 'pressure') {
  ...
  return augmentSystemPromptWithTerminology(
    basePrompt
