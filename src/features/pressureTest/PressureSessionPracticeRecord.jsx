/** @type {Record<import('./pressureTypes.js').QuestionRound['tone'], string>} */
const TONE_LABEL = { mild: '温和', direct: '直接', pressure: '加压' }

/**
 * @param {object} props
 * @param {import('./pressureTypes.js').PressureSession|null|undefined} props.session
 */
export function PracticeRecordSection({ session }) {
        ...
        {rounds.map((r) => (
          <div key={r.roundIndex}>
            <p className="text-xs font-semibold" style={{ color: 'var(--wb-text)' }}>
              ...
              <span className="ml-2 font-normal opacity-70">（{TONE_LABEL[r.tone] || r.tone}）</span>
            </p>
