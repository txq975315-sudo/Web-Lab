...
function highlightMatch(text, query) {
  ...
  return (
    <>
      {text.slice(0, idx)}
      <span style={{ backgroundColor: '#FEF08A' }}>{text.slice(idx, idx + query.length)}</span>
...
function SearchIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
      ...
      <path d="M11 11L14.5 14.5" stroke="#9CA3AF" strokeWidth="1.3" strokeLinecap="round" />
    </svg>
  )
}
...
function EnterIcon() {
  return (
    <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
      <path d="M9 2V7.5C9 8.32843 8.32843 9 7.5 9H3.5" stroke="#9CA3AF" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round" />
      <path d="M5 7L3 9L5 11" stroke="#9CA3AF" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  )
}
      ...
      >
        <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-100">
          ...
          <span className="text-[10px] text-gray-400 flex items-center gap-1 flex-shrink-0">
            <span className="px-1.5 py-0.5 rounded bg-gray-100 text-gray-500 font-mono text-[9px]">ESC</span>
            关闭
          </span>
        </div>
        ...
        <div ref={listRef} className="flex-1 overflow-auto py-2">
          {!searchQuery && !isProjectMode && !isNewMode && (
            <div>
              {showRecent && (
                <div>
                  <div className="px-4 py-1.5 text-[10px] text-gray-400 uppercase tracking-wider flex items-center gap-1.5">
                    <ClockIcon /> 最近访问
                  </div>
                  {recentDocs.map((doc, i) => (
                    <div
                      ...
                      style={{
                        backgroundColor: selectedIndex === i ? '#F5F3FF' : 'transparent'
                      }}
                      onMouseEnter={(e) => { if (selectedIndex !== i) e.currentTarget.style.backgroundColor = '#F9FAFB' }}
                      onMouseLeave={(e) => { if (selectedIndex !== i) e.currentTarget.style.backgroundColor = 'transparent' }}
                    >
                      <span className="text-gray-400 flex-shrink-0"><DocIcon /></span>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm text-gray-700 truncate">{doc.name}</p>
                        <p className="text-[10px] text-gray-400">{doc.projectName}</p>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                {QUICK_ACTIONS.map((action, i) => {
                  ...
                  return (
                    <div
                      ...
                      className="flex items-center gap-3 px-4 py-2 cursor-pointer mx-1 rounded-lg"
                      style={{
                        backgroundColor: selectedIndex === idx ? '#F5F3FF' : 'transparent'
                      }}
                      onMouseEnter={(e) => { if (selectedIndex !== idx) e.currentTarget.style.backgroundColor = '#F9FAFB' }}
                      onMouseLeave={(e) => { if (selectedIndex !== idx) e.currentTarget.style.backgroundColor = 'transparent' }}
                    >
                      <span className="text-base flex-shrink-0">{action.icon}</span>
                      <span className="text-sm text-gray-700">{action.label}</span>
                      <span className="text-[10px] text-gray-400 ml-auto">New:</span>
                    </div>
                  )
                })}
              </div>
          ...
          {showDocs && (
            <div>
              <div className="px-4 py-1.5 text-[10px] text-gray-400 uppercase tracking-wider">文档</div>
          ...
          {showProjects && (
            <div>
              <div className="px-4 py-1.5 text-[10px] text-gray-400 uppercase tracking-wider">项目</div>
              {filteredProjects.map((project, i) => {
                ...
                return (
                  <div
                    ...
                    onMouseLeave={(e) => { if (selectedIndex !== idx) e.currentTarget.style.backgroundColor = 'transparent' }}
                  >
                    <span className="text-gray-400 flex-shrink-0"><ProjectIcon /></span>
                    <span className="text-sm text-gray-700">
                      {highlightMatch(project.name, searchQuery)}
                    </span>
                    {project.id === activeProjectId && (
                      <span className="text-[10px] text-purple-400 ml-auto">当前</span>
                    )}
                  </div>
                )
              })}
          ...
          {showActions && (
            <div>
              <div className="px-4 py-1.5 text-[10px] text-gray-400 uppercase tracking-wider">创建</div>
          ...
          {!showRecent && !showDocs && !showProjects && !showActions && searchQuery && (
            <div className="px-4 py-8 text-center">
              ...
              <p className="text-xs text-gray-300 mt-1">尝试其他关键词，或使用 New: 前缀创建新文档</p>
            </div>
          )}
        </div>
        ...
        <div className="px-4 py-2 border-t border-gray-100 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 text-[10px] text-gray-400">
            <span className="flex items-center gap-1">
              <span className="px-1 py-0.5 rounded bg-gray-100 text-gray-500 font-mono text-[9px]">↑↓</span>
              导航
            </span>
            <span className="flex items-center gap-1">
              <span className="px-1 py-0.5 rounded bg-gray-100 text-gray-500 font-mono text-[9px]">↵</span>
